class MiningBuilding2 extends GroundObject
{
  float _w;
  float _h;
  float block = 0;
  ...
  MiningBuilding2(float x, float h)
  {
    super(x);
  ...
  void setup()
  {
    fortBuilding = loadImage ("miningHouse2.png");
    
    fortBuilding2 = loadImage ("miningHouse2Red.png");
    door1 = loadImage ("miningHouse2Door.png");
    door2 = loadImage ("miningHouse2DoorRed.png");
    ...
    if (visible()) {
      ...
      if (width/2 > x + 475 )
      {
       ...
       else
       {
        ...
        if (width/2 >= x + 235 && width/2 < x + 310)
        {
          image (door1, x + 200, y + 242);


          if (mouseX >= x + 235 && mouseX < x + 310 ) {

            image (door2, x + 200, y + 242);
            if (mousePressed)
            {
              indoors = 0;
              openTimer = 0;

class MiningFort extends GroundObject
{
  float _w;
  float _h;
  float tarLock = 0;
 ...
 float fortType = .5;
  ...
  void draw()
  {
    ...
    if (visible()) {
      ...
      y -= _h;
      fill(80, 8, 10);
       image (button, x - 2600, y - 70);
        if (width/2 > x + 90 && width/2 < x + 160 && gateTimer < 200)
     {
       rightStop = 1;
       
    }
     else if (gateTimer < 200 && width/2 > x+ 70) 
     {
       rightStop = 0;
     }
       if (width/2 < x - 2550 && width/2 > x - 2600 && gateCutSceneLock == 0 )
      {
       gateTimer = 1;
       arrowX = -1000000;
       gateButton = 1;
